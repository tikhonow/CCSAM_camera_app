    ...
    var body: some View {
        NavigationView {
            ScrollView {
                VStack(spacing: 24) {
                    ...
                    VStack(alignment: .leading, spacing: 12) {
                        ...
                        VStack(spacing: 8) {
                            InfoRow(title: "Версия приложения", value: AppConstants.appVersion)
                            InfoRow(title: "Версия прошивки", value: AppConstants.firmwareVersion)
            ...
            .alert("Почта недоступна", isPresented: $showMailAlert) {
                ...
                Text("На этом устройстве не настроена почта. Пожалуйста, напишите нам на \(AppConstants.supportEmail)")
            }
        }
    }
}
...
struct MailComposeView: UIViewControllerRepresentable {
    func makeUIViewController(context: Context) -> MFMailComposeViewController {
        ...
        composer.setToRecipients([AppConstants.supportEmail])
        ...
        Версия приложения: CCSAM Camera v\(AppConstants.appVersion)
        Версия прошивки: \(AppConstants.firmwareVersion)
        Версия iOS: \(UIDevice.current.systemVersion)
        Модель устройства: \(UIDevice.current.model)
        LiDAR поддержка: \(ARWorldTrackingConfiguration.supportsSceneReconstruction(.mesh) ? "Поддерживается" : "Не поддерживается")
        """
